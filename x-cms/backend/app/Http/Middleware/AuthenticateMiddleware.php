...
class AuthenticateMiddleware
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        if (in_array((explode('/', $request->path())[2] ?? null), ['import-example', 'export', 'download', 'download-template', 'download-attachment'])) {
            return $next($request);
        }
        if (in_array((explode('/', $request->path())[1] ?? null), ['download', 'download-attachment'])) {
            return $next($request);
        }
